# sending the intermediate steps to Chat GPT + spinner
...
# Check if simplified_intermediate_steps column exists and add it if not
c.execute("PRAGMA table_info(interactions)")
columns = [column[1] for column in c.fetchall()]
if "simplified_intermediate_steps" not in columns:
    c.execute("ALTER TABLE interactions ADD COLUMN simplified_intermediate_steps TEXT")
conn.commit()
...
def save_interaction(
    ...
    c.execute(
        """
        INSERT INTO interactions (session_id, id, user_query, assistant_response, intermediate_steps, simplified_intermediate_steps)
        VALUES (?, ?, ?, ?, ?, ?)
    """,
        (
            session_id,
...
if user_query:
    ...
    # Save both the original intermediate steps and the simplified explanation
    save_interaction(
        session_id,
        question_id,
        user_query,
